...
for i=1:length(threshold_UMI)       % calculate the error rate for each UMI threshold
    num_false_positive = sum( max(barcodematrix(:,idx_injection),[],2)>threshold_injection & max(barcodematrix(:,idx_negative_ctrl),[],2)>threshold_UMI(i) ); % # of neurons with false positive projection: max injection UMI>50 AND max ctrl UMI > threshold
    num_total = sum( max(barcodematrix(:,idx_injection),[],2)>threshold_injection & max(barcodematrix(:,idx_target),[],2)>threshold_UMI(i) ); % # of projection neurons: max injection UMI>50 AND max target UMI > threshold
